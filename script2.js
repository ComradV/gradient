...
function getMarkCoords() {
    ...
    let textElement = document.getElementById('textElem');
    var range = document.createRange();
    let top = textElement.offsetTop;
    let paragraphs = textElement.children;
    let classes = [];
    for (let i = 0; i < paragraphs.length; i++) {
        let paragraph = paragraphs[i];
        let parChildren = paragraph.childNodes;
        let startNotSet = true;
        let start = 0, end = 0;
        for (let j = 0; j < parChildren.length; j++) {
            let node = parChildren[j];
            if (node.nodeName == 'SPAN') {
                // var rects = range.getClientRects();
                classes.push(...getMarks(node));
                // addMarks(node, (rects[0].top + rects[0].bottom) / 2 - top);
            } else { //it is text node
                if (startNotSet) {
                    startNotSet = false;
                    range.setStart(node, 0);
                }
                let firstDot = node.textContent.indexOf('.');
                let lastDot = node.textContent.lastIndexOf('.');
                if (firstDot >= 0 && classes.length > 0) {
                    range.setEnd(node, firstDot);
                    let rects = range.getClientRects();
                    start = rects[0].top;
                    end = rects[rects.length - 1].bottom;
                    addMarksToCoord(classes, (start + end) / 2 - top);
                }
                if (lastDot >= 0) {
                    if (node.textContent.length - 1 == lastDot) {
                        startNotSet = true;
                    } else {
                        range.setStart(node, lastDot+1);
                    }
                    classes = [];
                }
            }
    ...
    function addMarksToCoord(marksArray, coord) {
        for (let i = 0; i < marksArray.length; i++) {
            let markClass = marksArray[i];
